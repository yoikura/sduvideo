...
import {getUserPlayNum, getUserInfo, getUserLikeNum} from '../api/api'
// TODO 设置
export default {
  name: 'PersonSpace',
  components: {
  ...
  data: function () {
    return {
      user: null,
      currentModule1: this.$route.params.module ? this.$route.params.module : 'PSWork',
  ...
  methods: {
    ...
    async getUserLikeNum(){
      let res = await getUserLikeNum({uid: this.$route.query.uid})
      this.likeNum = res.data.likeNum
    },
    async getUserPlayNum(){
      let res = await getUserPlayNum({uid: this.$route.query.uid})
  ...
  <el-container style="background: rgba(0, 0, 0, 0.05);">
    ...
    <el-header style="height:auto">
      ...
      <div class="normal-row">
        <banner :user="currentUser"></banner>
      </div>
      <div class="normal-row">
        <div class="head-bar">
...
<!--            </el-tab-pane>-->
          ...
          <div class="noraml-ban right-ban">
            <div class="right-ban-item common-a"
              ...
              <div class="right-ban-num">{{ this.currentUser.following }}</div>
            </div>
            <div class="right-ban-item common-a"
                 @click=""
                 ref="fans">
              <div class="right-ban-name">粉丝数</div>
              <div class="right-ban-num">{{ this.currentUser.followed }}</div>
            </div>
            <div class="right-ban-item">
              <div class="right-ban-name">获赞数</div>
              <div class="right-ban-num">{{ this.likeNum }}</div>
            </div>
            <div class="right-ban-item">
              <div class="right-ban-name">浏览数</div>
              <div class="right-ban-num">{{ this.playNum }}</div>
            </div>
          </div>
        </div>
      </div>
