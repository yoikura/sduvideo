  ...
  <div class="work-card">
    <div class="img-div">
      ...
      <a href="#"
        ...
        <div class="info"
             id="mask">
          <div class="info-item">点赞：{{handleNum(work.likeNum)}}</div>
<!--          <div class="info-item">投币：{{handleNum(work.coin)}}</div>-->
          <div class="info-item">收藏：{{handleNum(work.favoriteNum)}}</div>
<!--          <div class="info-item">下载：{{handleNum(work.download)}}</div>-->
    ...
    <div v-if="showInfo">
      <div class="title"
           :title="work.title" @click="goDetail(work.vid)">{{work.vname}}</div>
      <div class="date">
        <span>
          <i class="el-icon-video-play"
             style="margin-right:5px;"></i>{{handleNum(work.playNum)}}
        </span>
