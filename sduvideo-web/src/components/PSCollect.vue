...
import {getFavoriteVideo, getUserInfo} from '../api/api'
export default {
  name: 'PSCollect',
  components: {
    CollectCard
  ...
  methods: {
    async getFavoriteVideo(){
      let res = await getFavoriteVideo({uid:this.user.uid})
      this.collectList[0].collects = res.data.data
    },
    doSelect (index) {
  ...
  async created () {
    ...
    await this.getFavoriteVideo()
    this.curCollects = this.initCurCollects()
  },
  beforeMount () {
  }
}
        ...
        <div v-if="curCollects.pageCount!=0">
          <div class="collect-info">
            ...
            <div class="collect-info-font">
              ...
              <div class="collect-info-i">创建者：{{this.currentUser.nickName}}</div>
