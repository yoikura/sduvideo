...
<template>
  <div class="collect-card">
    <div class="img-div">
      <img :src="collect.img"
           class="img-style"
          @click="goDetail(collect.id)"/>
      <a href="#"
         v-if="showInfo">
        <div class="info"
             id="mask">
          <div class="info-item">浏览：{{handleNum(collect.playNum)}}</div>
<!--          <div class="info-item">收藏：{{handleNum(collect.favoriteNum)}}</div>-->
          <div class="info-item">UP主：{{collect.author}}</div>
<!--          <div class="info-item">投稿：{{handleTime(collect.upTime)}}</div>-->
    ...
    <div v-if="showInfo">
      <div class="title"
           :title="collect.title" @click="goDetail(collect.id)">{{collect.name}}</div>
      <div class="date">{{handleTime(collect.createTime)}}
